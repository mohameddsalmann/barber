    ...
    <PageShell>
      <div className="px-5 pt-5">
        <div className="flex items-center gap-3">
          <img src={me.avatar} alt="" className="w-16 h-16 rounded-full" />
        ...
        <div className="mt-5 rounded-2xl p-[1.5px]" style={{ background: "linear-gradient(135deg, #D4AF37, #16161A 60%, #F43F5E)" }}>
          <div className="rounded-2xl bg-card p-5">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Loyalty Tier</div>
                <div className="font-display text-2xl mt-0.5 gold-text">VIP</div>
              </div>
              <TierBadge tier={me.tier} />
            </div>
            <div className="mt-3 flex items-end justify-between">
              <div>
                <div className="font-mono text-3xl text-primary">1,240</div>
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground">points</div>
              </div>
              <div className="text-right text-[11px] text-muted-foreground">{me.visits} visits<br />${me.ltv.toLocaleString()} lifetime</div>
            </div>
          </div>
        </div>

        <h2 className="mt-6 mb-2 text-sm uppercase tracking-wider text-muted-foreground">Visit history</h2>
        <div className="relative pl-5 border-l-2 border-primary/40 space-y-4">
          {visitHistory.map((v, i) => (
            <div key={i} className="relative">
              <span className="absolute -left-[27px] top-1 w-3 h-3 rounded-full bg-primary border-4 border-background" />
              <div className="text-[11px] font-mono text-muted-foreground">{v.date}</div>
              <div className="font-display text-sm">{v.service} · ${v.spent}</div>
              <div className="text-[11px] text-muted-foreground">with {v.barber}</div>
            </div>
          ))}
        </div>

        <h2 className="mt-6 mb-2 text-sm uppercase tracking-wider text-muted-foreground">Favorite barbers</h2>
        <div className="flex gap-3 overflow-x-auto scrollbar-hide -mx-5 px-5 pb-2">
          {barbers.map((b) => (
            <div key={b.id} className="shrink-0 w-32 bg-card border border-border rounded-xl p-3 text-center">
              <img src={b.avatar} alt="" className="w-14 h-14 rounded-full mx-auto" />
        ...
        <h2 className="mt-6 mb-2 text-sm uppercase tracking-wider text-muted-foreground">Payment</h2>
        <div className="bg-card border border-border rounded-xl p-4 flex items-center gap-3">
          ...
          <span className="px-2 py-0.5 rounded-full text-[10px]" style={{ backgroundColor: "rgba(212,175,55,0.15)", color: "#D4AF37" }}>Default</span>
        </div>
      </div>
    </PageShell>
  );
